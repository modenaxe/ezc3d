...
bool ezc3d::ParametersNS::GroupNS::Group::isLocked() const { return _isLocked; }

void ezc3d::ParametersNS::GroupNS::Group::lock() { _isLocked = true; }
void ezc3d::ParametersNS::GroupNS::Group::unlock() { _isLocked = false; }
...
ezc3d::ParametersNS::GroupNS::Group::parameter(size_t idx) {
  try {
    ...
    throw std::out_of_range(
        "Groups::parameter method is trying "
        "to access the parameter " +
        std::to_string(idx) + " while the maximum number of parameters is " +
        std::to_string(nbParameters()) + " in the group " + name() + ".");
  }
}
...
void ezc3d::ParametersNS::GroupNS::Group::remove(const std::string &name) {
  remove(this->parameterIdx(name));
}

void ezc3d::ParametersNS::GroupNS::Group::remove(size_t idx) {
  if (idx >= nbParameters()) {
    throw std::out_of_range(
        "Groups::parameter method is trying "
        "to access the parameter " +
